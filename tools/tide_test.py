...
def main():
    ...
    if args.work_dir is not None and rank == 0:
        ...
        if args.out is not None:
            name = f'{args.out.split(".")[0]}_eval.json'
            json_file = osp.join(args.work_dir, name)
        else:
    ...
    #                              args.gpu_collect)
    outputs = mmcv.load('/home/tju531/hwr/mmdetection/tools/others/results/4_retinanet_r50_fpn_1x_fgvoc/0_raw.pkl')
    rank, _ = get_dist_info()
    if rank == 0:
        ######不需要写成.pkl文件
        # if args.out:
        ...
        #         out = osp.join(args.work_dir, args.out)
        #         print('-'*30)
        #         print(out)
        ...
        if json is not None:
            if args.work_dir is not None:
                json_name = osp.join(args.work_dir, json)
            dataset.format_results(outputs, json_name)  #####################
        #   voc类型的dataset没有format_results 照抄了coco数据类型
        if args.eval:
        ...
        if args.tide:
            ...
            if args.work_dir is not None:
                tide_json = osp.join(args.work_dir, f'{json_name}_tide_error.json')
                output_img_path = osp.join(args.work_dir, 'tide_img')
            else:
                tide_json = f'{json_name}_tide_error.json'
                output_img_path = 'tide_img'
            mmcv.dump(errors, tide_json)
            tide.plot(out_dir=output_img_path)
