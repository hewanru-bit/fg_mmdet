...
def imgpath2txt(root_path,txt_path):
    # train/a  ,train/b.... write into train.txt
    img_dir = os.listdir(root_path)
    for k in range(len(img_dir)):
        txt_f = open(txt_path, "a+")
        print('begin {}'.format(img_dir[k]))
        img_path = os.path.join(root_path,img_dir[k])
        file_list = []  # 建立列表，用于保存图片信息
        imgs = os.listdir(img_path)
        print('此文件夹下共有{}张图片'.format(len(imgs)))
        i = 0
        ...
        for file in imgs:  # file为current_dir当前目录下图片名
            if file.endswith('.png') or file.endswith(".jpg"):  # 如果file以jpg或者png结尾
                i += 1
                write_name = file  # 文件名+后缀
                # write_name = file.split('.png')[0]  # 不加后缀
                file_list.append(write_name)  # 将write_name添加到file_list列表最后
            sorted(file_list)  # 将列表中所有元素排列,但是如果是1，10，2，的话并不会变成1，2，10
            number_of_lines = len(file_list)  # 列表中元素个数
            # 将图片信息写入txt文件中，逐行写入
        print('已完成{}张图片写入.txt'.format(i))
        for current_line in range(number_of_lines):
            txt_f.write(img_dir[k]+'/'+file_list[current_line] + '\n')  # 关闭文件

    print('finish!!!')


def main():
    img_path = '/home/tju531/hwr/Datasets/cityscape/train/'
    txt_path = '/home/tju531/hwr/Datasets/cityscape/train.txt'
    imgpath2txt(img_path, txt_path)
