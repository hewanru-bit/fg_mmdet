    ...
    for _ in range(random_n):
        ...
        for anchor in anchors:
            ...
            index = (anchor[:, 0] > 0) & (anchor[:, 1] > 0) & (anchor[:, 2] < input_shape_hw[1]) & \
                    (anchor[:, 3] < input_shape_hw[0])    # 得到一些valid的anchor
            anchor = anchor[index]
            anchor = np.random.permutation(anchor)  # np.random.permutation()：随机排列序列
            img_ = show_bbox(img, anchor[:select_n], thickness=1, is_show=False)
            disp_img.append(img_)
        show_img(disp_img, stride)
