...
def main_analysis(config,pkl,show_dir):
    args = parse_args()

    args.config = config
    args.prediction_path=pkl
...
def main():
    '''根据config文件和.pkl文件，生成测试图片带bbox框的测试结果图片，存放在show_dir文件下，源文件为analyze_results'''
                ...
                if model_name in pkl_list[j].split('_'): ###得到该Model pkl的文件夹pkl_list[j]
                    ...
                    for k in range(len(enh_pkl_list)):
                        ...
                        if enh_name in select_enhan:  ##选择挑选的增强方法.pth
                            ...
                            save_name = pkl_list[j]+'/'+enh_name+'/'   ### model/enhance
                            show_dir = osp.join(show_base,save_name)
                            print('开始处理{}模型的{}处理方法'.format(model_name,enh_name))
                            main_analysis(config,pkl,show_dir)
