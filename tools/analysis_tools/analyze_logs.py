...
def add_time_parser(subparsers):
    ...
    parser_time.add_argument(
        ...
        type=str,
        nargs='+',
        default='/home/tju531/hwr/mmdetection/tools/others/work_dirs/faster_r50_fpn_1x_fgvoc/',
        help='path of train log in json format')
    parser_time.add_argument(
        '--include-outliers',
        action='store_true',
...
def main():
    args = parse_args()
    # 使用json所在的文件夹的路径
    # 多个json的话，需要len(legend) == (len(args.json_logs) * len(args.keys))
    # json_list = os.listdir(args.json_logs)
    # json_logs = []
    # for json_log in json_list:
    #     if json_log.endswith('.json'):
    #         json_logs.append(os.path.join(args.json_logs,json_log))
    # args.json_logs = [json_logs]

   # 单个json args.json_logs路径写到.json
    args.json_logs=[args.json_logs]
