...
def reshape_transform(feats, max_shape=(20, 20), is_need_grad=False):
    ...
    actis = []
    for feat in feats:
        actis.append( torch.nn.functional.interpolate(
                torch.abs(feat), max_shape, mode='bilinear'))

    actis = torch.cat(actis, axis=1)
    return actis
...
class DetAblationLayer(AblationLayer):
    ...
    def set_next_batch(self, input_batch_index, activations,num_channels_to_ablate):
        ...
        if isinstance(activations, torch.Tensor):
            return super(DetAblationLayer, self).set_next_batch(input_batch_index, activations,
