...
class NBCFPN(BaseModule):
    """BFP cfpn的结合
    bcfpn,但是像原本的retinanet中的fpn，5层输出，处理前三层，两个额外的层不进行cat处理，但是最后进行的dp处理
    """
    ...
    def __init__(self,
        ...
        super(NBCFPN, self).__init__(init_cfg)
        ...
        self.fc = nn.Sequential(nn.AdaptiveAvgPool2d(1),
                                nn.Conv2d(self.out_channels*(4-self.start_level), (4-self.start_level), 1)
                                )
        ...
        if self.cat_feats:
            self.fp1_convs = ConvModule(
                    out_channels*(4-self.start_level),
                    out_channels,
                    3,
                    stride=1,
                    padding=1,
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            prev_shape = lat[i - 1].shape[2:]  # 得到每层laterals的H，W
