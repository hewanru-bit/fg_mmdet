...
from mmcv.ops.merge_cells import GlobalPoolingCell

from ..builder import NECKS


@NECKS.register_module()
class BCFPNC(BaseModule):
    """
    PANet增加down_up解释说更有利于融合浅层特征，同时浅层对定位很重要，所以这里采用先down-up,然后上下采样到统一的尺寸，torch.cat，得到
    one_feats,分别与对应层经过nas_fpn中的GlobalPoolingCell，再插值回原来的尺寸，然后up-down
    """
    ...
    def __init__(self,
        ...
        super(BCFPNC, self).__init__(init_cfg)
    ...
    def forward(self, inputs):
        ...
        one_feat = sum([t_outs[i]*w[i] for i in range(self.num_outs)])
        tmps=[]
        for i in range(0, self.num_outs):
            tmps.append(self.cell( one_feat,t_outs[i], out_size=t_outs[i].shape[-2:]))
