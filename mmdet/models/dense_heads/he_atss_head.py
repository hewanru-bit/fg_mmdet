...
'''HEATSSHead中新加了edge分支'''
@HEADS.register_module()
class HEATSSHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(HEATSSHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def forward_single(self, x, scale):
        ...
        return cls_score, bbox_pred, centerness,edge_imgs
    ...
    def forward_train(self,
        ...
        """
        outs = self(x)   ## outs是一个list,里面是检测头的输出，每一个又有5层edge_imgs=outs[3]
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas,gt_edge)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas,gt_edge)  ###loss_inputs=outs[3]
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'centernesses','edge_imgs'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
        ...
        ###########求edge loss #########################
        edge_target = self.edge_target(edge_imgs[0], gt_edge)
    ...
    ####重写 self.bbox_head.simple_test,添加参数,gt_edge，原型在BBoxTestMixin类中
    def simple_test_bboxes(self,gt_edge,feats, img_metas, rescale=False):
