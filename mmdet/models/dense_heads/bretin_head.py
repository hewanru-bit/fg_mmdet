...
'''在ARetinaHead 的基础上添加 centerness 分支,该分支与fcos中和atss中的一样，放在reg或者clc分支。
该检测头有centerness和edge两个分支，edge分支是独立的'''
@HEADS.register_module()
class BRetinaHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(BRetinaHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def loss_single(self, anchors, cls_score, bbox_pred,  centernesses, labels,
        ...
        centerness = centernesses.permute(0, 2, 3, 1).reshape(-1)
    ...
    def centerness_target(self, anchors, gts):
        ...
        l_ = abs(anchors_cx - gts[:, 0])
        t_ = abs(anchors_cy - gts[:, 1])
        r_ = abs(gts[:, 2] - anchors_cx)
        b_ = abs(gts[:, 3] - anchors_cy)
