...
'''ARetinaNet 检测器,ARetinaHead添加边缘分支'''
# model settings
model = dict(
    type='ARetinaNet',
    backbone=dict(
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=1.0),
        loss_bbox=dict(type='L1Loss', loss_weight=1.0),  # 更换了 原来是l1 loss w=1.0
        # loss_bbox=dict(type='GIoULoss', loss_weight=2.0),
        # 添加了影响bbox的分支centerness或者distance分支，需要用GIoULoss 原来是l1 loss回报错
        # loss_distance=dict(type='CrossEntropyLoss',
...
train_pipeline = [
    ...
    dict(type='GetEdge',only_bbox=True),  ### only_bbox = True 是只求bbox中 目标的边缘，Fasle时求整张图片的边缘
